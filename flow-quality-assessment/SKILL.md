...
- Consulte o Nu Design System via o conector disponível na sessão (**zeroheight/MCP** ou **Glean**, que indexa a documentação interna).
- Se nenhuma fonte de NuDS estiver acessível na sessão (ex.: página atrás de SSO, conector desligado), **sinalize ao usuário** e avalie apenas com base no que é observável no fluxo, marcando os parâmetros dependentes de NuDS como **"sujeito a validação contra NuDS v3"** no rationale. Não invente regras do design system.
...
Quando o fluxo vier por Figma, liste os componentes NuDS V3 detectados e **cruze com a documentação vigente do NuDS** (conector zeroheight/Glean) para flagar deprecados ou substituídos. A lista de deprecados **não é congelada nesta skill** — muda com o design system. Se a fonte NuDS não estiver acessível na sessão, informe que o flag de deprecados não pôde ser validado e siga avaliando o observável. Componentes deprecados detectados pesam em **Brand & Content Compliance** (e podem afetar **Navigation & Flow Logic** se o pattern estiver fora do padrão atual).
